...
async function assertScreen(page, name) {
  ...
  const detail = page.locator('.chart-analysis-table, .summary-table-wrap').filter({ visible: true }).first();
  ...
  try {
    ...
    for (const width of [1440, 768, 390]) {
      for (const theme of ['dark', 'light']) {
        ...
        await assertScreen(page, `${width}-${theme}-summary`);
        await page.getByRole('radiogroup', { name: '화면 선택', exact: true }).getByText('차트분석', { exact: true }).click();
        await page.locator('.chart-analysis-title').waitFor();
        await page.getByText('전체 종목 점수', { exact: true }).waitFor();
        await assertScreen(page, `${width}-${theme}-chart`);
        await page.getByRole('radiogroup', { name: '테마', exact: true }).getByText(theme === 'dark' ? '라이트' : '다크', { exact: true }).click();
        await page.locator('.chart-analysis-title').waitFor();
        assert.ok(await page.getByRole('radio', { name: '차트분석', exact: true }).isChecked());
        await assertScreen(page, `${width}-${theme}-theme-switch`);
        await page.close();
      }
    }
    for (const scenario of ['login', 'empty', 'partial']) {
      ...
      await page.getByRole('radio', { name: '라이트', exact: true }).waitFor();
      if (scenario === 'login') await page.getByRole('checkbox', { name: '로그인 유지', exact: true }).waitFor();
      ...
      await assertScreen(page, `390-${scenario}`);
      await page.close();
    }

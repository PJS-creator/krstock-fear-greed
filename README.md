...
3. **New app**을 클릭합니다.
4. **Repository**에서 이 GitHub 저장소를 선택합니다.
5. **Branch**는 `main`을 선택합니다.
6. **Main file path**에는 `app/portfolio_dashboard.py`를 입력합니다.
7. **Python version**은 `3.11` 또는 `3.12`를 선택합니다.
8. **Advanced settings / Secrets**는 현재 MVP에서는 비워 둡니다. `.env`, API key, 개인정보 파일은 필요하지 않습니다.
9. **Deploy**를 클릭합니다.

Streamlit Cloud는 대시보드 앱 디렉터리의 `app/requirements.txt`를 사용해 `streamlit`, `pandas`를 설치할 수 있습니다. root `requirements.txt`는 기존 DCInside scraper용으로 가볍게 유지합니다.

배포 후 URL은 보통 아래와 같은 형태입니다.

```text
https://<app-name>.streamlit.app
```

또는 Streamlit이 GitHub 저장소 이름을 기반으로 생성한 앱 이름을 포함해 표시합니다.

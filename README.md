...
v0.9부터 기본 입력 화면에서는 종목명 또는 티커, 수량, 선택 평단가를 입력합니다. `market`, `currency`, `display_name`, 현재가, 전일종가는 기본 입력에서 숨기고 앱이 가능한 범위에서 자동으로 채웁니다.
...
평가이익과 수익률은 **보유자산 → 빠른 입력** 또는 **고급 설정**에서 입력한 `avg_price` 기준입니다. 평단가가 없는 종목은 잘못된 0% 수익률로 표시하지 않고 미산정으로 남깁니다.

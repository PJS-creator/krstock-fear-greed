...
미국 주식 가격 조회는 yfinance를 사용하며 별도 API key가 필요 없습니다. 국내 주식 가격 조회는 FinanceDataReader를 사용하며 별도 API key가 필요 없습니다. Alpha Vantage key는 USD/KRW 환율 갱신 버튼을 사용할 때만 필요합니다.
...
- `missing`: 가격 데이터 없음
- `missing_api_key`: 이전 버전의 Alpha Vantage API key 없음 상태
- `manual`: 수동 가격 유지
...
총현금은 KRW 기준으로 환산되어 표시됩니다. **USD/KRW 환율 갱신** 버튼을 누르면 Alpha Vantage `CURRENCY_EXCHANGE_RATE`로 USD/KRW를 조회합니다. 이 기능을 쓰려면 Streamlit Secrets에 `ALPHA_VANTAGE_API_KEY`가 필요합니다. 실패하면 기존 수동 환율을 유지합니다. 환율 조회도 자동 실행되지 않고 버튼을 눌렀을 때만 실행됩니다.
...
미국 주식 가격 새로고침만 사용할 경우 `ALPHA_VANTAGE_API_KEY`는 비워도 됩니다. 이 key는 사이드바의 **USD/KRW 환율 갱신** 버튼을 사용할 때만 필요합니다.

...
Streamlit 대시보드는 Cloud 프론트엔드 호환성을 위해 `streamlit==1.58.0`으로 고정합니다. `app/requirements.txt`, `requirements-dashboard.txt`, `requirements-dev.txt`의 대시보드 핵심 버전은 함께 맞춥니다.
...
Secrets를 수정했거나 `app/requirements.txt`가 바뀐 경우 Streamlit Cloud에서 앱을 Reboot 또는 Redeploy 해야 새 설정과 의존성이 반영됩니다. 대시보드는 `streamlit==1.58.0`으로 고정되어 있으므로 merge 후 Reboot 또는 Redeploy가 필요합니다.
...
테스트에는 formatter/status/chart 순수 함수 테스트와 `streamlit.testing.v1.AppTest` 기반 smoke test가 포함됩니다. AppTest는 핵심 KPI 4개, 기본 탭, 가격 로그 상세 expander, raw ISO 시간 미노출을 확인합니다. `requirements-dev.txt`에는 이 테스트를 위해 `streamlit==1.58.0`이 포함됩니다.

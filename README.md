...
API 또는 데이터 소스 호출은 일반적인 Streamlit 화면 rerun만으로 반복 실행되지 않습니다. 로그인 후 계정의 기본 포트폴리오가 자동으로 불러와진 경우에는 USD/KRW 환율과 보유 종목 가격을 한 번 자동 갱신합니다. 그 외에는 사용자가 **가격 새로고침** 또는 **USD/KRW 환율 갱신**을 누른 경우에만 실행됩니다.
...
5. 공용 로그인/회원가입 앱을 배포할 경우 `docs/supabase_migration_v1_public_accounts.sql` 파일의 내용도 복사해 실행합니다.
...
공용 앱은 첫 화면에 **로그인**과 **회원가입** 탭을 표시합니다. 사용자가 회원가입하면 `public_accounts` 테이블에 아이디와 비밀번호 해시가 저장되고, 앱은 `public:<아이디>` 값을 `owner_id`로 사용해 해당 사용자 포트폴리오만 저장/불러옵니다.

공용 앱 배포 전 Supabase SQL Editor에서 `docs/supabase_migration_v1_public_accounts.sql` 파일의 SQL 전체를 복사해 실행합니다. 파일 경로를 입력하지 말고, SQL 내용을 붙여넣은 뒤 **Run**을 누릅니다.
...
공용 앱에는 `APP_PASSWORD`, `ACCOUNTS`, `PORTFOLIO_OWNER_ID`를 넣지 않아도 됩니다. 가입한 사용자마다 `owner_id`가 자동으로 분리됩니다.
...
- 개인 테스트 앱은 단일 사용자 설정을 유지합니다. 외부 사용자용 앱은 `public_accounts` 기반 로그인/회원가입과 사용자별 `owner_id` 분리를 사용합니다. OAuth와 팀 계정은 이번 버전 범위 밖입니다.
